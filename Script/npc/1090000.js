...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			if (cm.getJobId().equals(MapleJob.BEGINNER)) {
				if (cm.getLevel() >= 10)
				...
				if (cm.getLevel() >= 30 && cm.getJobId().equals(MapleJob.PIRATE)) {
					status = 10;
					cm.sendNext("The progress you have made is astonishing.");
				} else if (cm.getLevel() >= 70 && (cm.getJobId().equals(MapleJob.GUNSLINGER) || cm.getJobId().equals(MapleJob.BRAWLER))) {
					cm.sendOk("Please go visit #bPedro#k. He resides in #bEl Nath#k.");
					cm.dispose();
				} else if (cm.getLevel() < 30 && cm.getJobId().equals(MapleJob.PIRATE)) {
					cm.sendOk("Please come back to see me once you have trained more.");
					cm.dispose();
				} else if (cm.getLevel() >= 120 && (cm.getJobId().equals(MapleJob.OUTLAW) || cm.getJobId().equals(MapleJob.MARAUDER))) {
