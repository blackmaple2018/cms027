...
function action(mode, type, selection) {
	...
	if (status <= 0) {
       var selStr = " #i4030000#  #i4030001#  #i4030010# #r#e< 每日必做 >#k#n #i4030011#  #i4030014#  #i4030015#\r\n\r\n";
		...
		selStr += "#b任务简介#k；前往魔法密林码头，坐一次飞往天空之城的航班即可完成任务。如果遭遇蝙蝠魔攻击可获取更多奖励。\r\n#b任务奖励#k；卷轴，魔法石，制作卷 \r\n\r\n";
			
		if (cm.getPlayer().getbosslog("每日任务_坐密林和天空的航班") <= 0) {
			selStr += "\t  #b码头航班#k 未完成 \r\n";
		}else{
			selStr += "\t  #b码头航班#k 已完成 \r\n";
		}
		
		if (cm.getPlayer().getbosslog("每日任务_航行遭遇蝙蝠魔袭击") <= 0) {
			selStr += "\t  #b航班危机#k 未遇到 \r\n";
		}else{
			selStr += "\t  #b航班危机#k 已遇到 \r\n";
		}
        ...
        switch (selection) {
			case 3:	
				if(cm.getPlayer().getbosslog("每日任务_坐密林和天空的航班完成") <= 0){
					if(现金券 >= 50){
						任务奖励(5);
						cm.setbosslog("每日任务_坐密林和天空的航班完成");
