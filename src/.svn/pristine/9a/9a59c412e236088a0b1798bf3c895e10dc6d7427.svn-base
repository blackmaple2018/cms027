...
public class ItemPet implements Serializable {
    ...
    public final void saveDatabase() {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement ps = con.prepareStatement("UPDATE pets SET name = ?, level = ?, closeness = ?, fullness = ?, seconds = ?, excluded = ? WHERE petid = ?")) {
                ps.setString(1, name);
                ps.setByte(2, level);
    ...
    public static ItemPet createPet(int itemid, String name, int level, int closeness, int fullness, int uniqueid, int secondsLeft) {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement pse = con.prepareStatement("INSERT INTO pets (petid, name, level, closeness, fullness, seconds) VALUES (?, ?, ?, ?, ?, ?)")) {
    ...
    public final void setExpiration(final long s) {
        this.expiration = s;
    }
