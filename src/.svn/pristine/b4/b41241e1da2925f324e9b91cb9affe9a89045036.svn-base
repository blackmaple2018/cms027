    ...
    public synchronized void addPoints(Client c, int points, long expiration, String reason) {
        ...
        if (this.points.get(acc) >= autobanPoints) {
            ...
            if (GameConstants.AUTO_BAN) {
                c.getPlayer().ban(banReason.toString(), false);
                BroadcastService.broadcastGMMessage(c.getWorld(), PacketCreator.ServerNotice(6, name + " 已经被系统禁止了。 (原因: " + reason + ")"));
                FileLogger.logToFile(FileLogger.FILE_PATH + "/autoban.txt", name + " : " + reason);
            }
            return;
        }
        if (expiration > 0) {
