...
public class MaplePacketDecoder extends CumulativeProtocolDecoder {
    ...
    protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) {
        ...
        if (in.remaining() >= 4 && decoderState.packetlength == -1) {
            ...
            if (!passCheck) {
                System.out.println("Closing session in decode " + HexTool.toString(in.array()));
                session.close();
                return false;
            }
        ...
        if (in.remaining() >= decoderState.packetlength) {
            ...
            int op = decryptedPacket[0] & 0xFF;

            if (op != 0x5F && op != 0x18 && op != 0x12 && op != 0x5A && op != 0x2F && op != 0x4C && op != 0x52) {
                System.out.println("recv：" + HexTool.toString(decryptedPacket));
            }
