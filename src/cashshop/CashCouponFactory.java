...
public class CashCouponFactory {
    ...
    public static boolean getCouponCodeValid(String code) {
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("SELECT `used` FROM `cashshopcouponitems` WHERE `code` = ?");
            ps.setString(1, code);
    ...
    public static List<CashCouponData> getCcData(final String code) {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT `type`, `itemData`, `quantity` FROM `cashshopcouponitems` WHERE `code` = ?")) {
                ps.setString(1, code);
    ...
    public static void setCouponCodeUsed(String name, String code) {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement ps = con.prepareStatement("UPDATE `cashshopcouponitems` SET `character` = ?, `used` = 1 WHERE code = ?")) {
                ps.setString(1, name);
                ps.setString(2, code);
                ps.execute();
            }
    ...
    public static void deleteCouponData(String name, String code) {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement ps = con.prepareStatement("DELETE from `cashshopcouponitems` WHERE `code` = ?")) {
                ps.setString(1, code);
                ps.execute();
            }
        } catch (SQLException e) {
