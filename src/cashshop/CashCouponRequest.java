...
public class CashCouponRequest {
    ...
    public static void CouponCode(InPacket packet, Client c) {
        packet.readShort();
        String code = packet.readMapleAsciiString();        
        
        if (code == null || code.length() < 16 || code.length() > 32) { 
            c.write(CashShopPackets.CouponError(CashShopPackets.ERROR_COUPON_NUMBER));
            return;
        }
        ...
        if (c.getPlayer().getCashShop().getItemsSize() >= (100 - cscsize.getLeft().getRight())) {
            c.write(CashShopPackets.CouponError(CashShopPackets.ERROR_INVENTORY_FULL));
            return;
        }
        ...
        c.write(CashShopPackets.CouponRewards(c.getAccountID(), maplePoints, togiveCS, togiveII, mesos));
