...
public class RockPaperScissors {
    ...
    public RockPaperScissors(final Client c, final byte mode) {
        c.write(PacketCreator.GetRockPaperScissorsMode((byte) (0x09 + mode), -1, -1, -1));
        if (mode == 0) {
    ...
    public final boolean answer(final Client c, final int answer) {
        if (ableAnswer && !win && answer >= 0 && answer <= 2) {
            ...
            if (response == answer) {
                c.write(PacketCreator.GetRockPaperScissorsMode((byte) 0x0B, -1, (byte) response, (byte) round));
            } else if ((answer == 0 && response == 2) || (answer == 1 && response == 0) || (answer == 2 && response == 1)) {
                c.write(PacketCreator.GetRockPaperScissorsMode((byte) 0x0B, -1, (byte) response, (byte) (round + 1)));
                ableAnswer = false;
                win = true;
            } else { 
                c.write(PacketCreator.GetRockPaperScissorsMode((byte) 0x0B, -1, (byte) response, (byte) -1));
                ableAnswer = false;
            }
            return true;
        }
    ...
    public final boolean timeOut(final Client c) {
        if (ableAnswer && !win) {
            ableAnswer = false;
            c.write(PacketCreator.GetRockPaperScissorsMode((byte) 0x0A, -1, -1, -1));
            return true;
        }
        reward(c);
        return false;
    ...
    public final boolean nextRound(final Client c) {
        if (win) {
            ...
            if (round < 10) {
                win = false;
                ableAnswer = true;
                c.write(PacketCreator.GetRockPaperScissorsMode((byte) 0x0C, -1, -1, -1));
                return true;
            }
        }
        reward(c);
