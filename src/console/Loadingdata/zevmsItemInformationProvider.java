...
public class zevmsItemInformationProvider {

    private static zevmsItemInformationProvider instance = null;

    protected MapleDataProvider itemData;
    protected MapleDataProvider equipData;
    protected MapleDataProvider stringData;
    protected MapleData itemStringData;
    ...
    public static void main(String args[]) {
        System.err.println("开始载入物品代码。");
        zevmsItemInformationProvider.getInstance().getAllItems();
    }

    protected zevmsItemInformationProvider() {
        ...
        itemStringData = stringData.getData("Item.img");
    ...
    public static zevmsItemInformationProvider getInstance() {
        if (instance == null) {
            instance = new zevmsItemInformationProvider();
        }
        return instance;
    }
    ...
    public LinkedHashMap<Integer, String> getAllItems() {
        ...
        MapleData[] itemsData = {
            stringData.getData("Item.img").getChildByPath("Eqp"),
            stringData.getData("Item.img")
        };
            ...
            for (MapleData itemFolder : itemsData[i].getChildren()) {
                if (i == 0) { //eqp, needs to traverse its child paths
                    for (MapleData equipItemFolder : itemFolder.getChildren()) {
                        itemIdAndName.put(Integer.parseInt(equipItemFolder.getName()), MapleDataTool.getString("name", equipItemFolder, "NO-NAME"));
                        //if (Integer.parseInt(equipItemFolder.getName()) > 1050107 && !isCash(Integer.parseInt(equipItemFolder.getName()))) {
                        Connection con = Start.getInstance().getConnection();
                        try {
                            PreparedStatement psu = con.prepareStatement("INSERT INTO wz_mxditemid (name, itemid,job,level) VALUES (?,?,?,?)");
                            psu.setString(1, MapleDataTool.getString("name", equipItemFolder, "NO-NAME"));
                            psu.setInt(2, Integer.parseInt(equipItemFolder.getName()));
                            //getEquipStats(Integer.parseInt(otherString.getName())).get("reqjob")
                            psu.setInt(3, 0);
                            psu.setInt(4, 0);
                            psu.executeUpdate();
                            psu.close();
                    ...
                    if ("Eqp".equalsIgnoreCase(itemFolder.getName())) {
                        continue;
                    }
                    for (MapleData otherString : itemFolder.getChildren()) {
                        itemIdAndName.put(Integer.parseInt(otherString.getName()), MapleDataTool.getString("name", otherString, "NO-NAME"));
    ...
    protected MapleData getStringData(int itemId) {
        String cat = "null";
        MapleData theData = itemStringData;
        String fix = "Eqp";
        ...
        if ("Eqp".equalsIgnoreCase(fix)) {
            return theData.getChildByPath(fix + "/" + cat + "/" + itemId);
        } else {
            return theData.getChildByPath(fix + "/" + itemId);
        }
    }
    ...
    public int 基础属性(int itemId, String a) {
        Integer atk = projectileWatkCache.get(itemId);
        if (atk != null) {
            return atk;
        }
        MapleData data = getItemData(itemId);
        atk = MapleDataTool.getInt("info/" + a + "", data, 0);
        return atk;
    }
    ...
    public Equip randomizeStatsIncuba(Equip equip) {
        if (equip.getStr() > 0) {
            equip.gainStrPoints((int) (Math.random() * 20));
        }
        if (equip.getDex() > 0) {
            equip.gainDexPoints((int) (Math.random() * 20));
        }
        if (equip.getInt() > 0) {
            equip.gainIntPoints((int) (Math.random() * 20));
        }
        if (equip.getLuk() > 0) {
            equip.gainLukPoints((int) (Math.random() * 20));
        }
        if (equip.getMatk() > 0) {
            equip.gainMatkPoints((int) (Math.random() * 20));
        }
        if (equip.getWatk() > 0) {
            equip.gainWatkPoints((int) (Math.random() * 20));
        }
        if (equip.getAcc() > 0) {
            equip.gainAccPoints((int) (Math.random() * 20));
        }
        if (equip.getAvoid() > 0) {
            equip.gainAvoidPoints((int) (Math.random() * 20));
        }
        if (equip.getJump() > 0) {
            equip.gainJumpPoints((int) (Math.random() * 20));
        }
        if (equip.getSpeed() > 0) {
            equip.gainSpeedPoints((int) (Math.random() * 40));
        }
        if (equip.getWdef() > 0) {
            equip.gainWdefPoints((int) (Math.random() * 40));
        }
        if (equip.getMdef() > 0) {
            equip.gainMdefPoints((int) (Math.random() * 40));
        }
        if (equip.getHp() > 0) {
            equip.gainHpPoints((int) (Math.random() * 40));
        }
        if (equip.getMp() > 0) {
            equip.gainMpPoints((int) (Math.random() * 40));
        }
        return equip;
    }
    ...
    public boolean isExpOrDropCardTime(int itemId) {
        ...
        if (getExpCardTimes.containsKey(itemId)) {
            ...
            data.stream().map((childdata) -> MapleDataTool.getString(childdata).split(":")).forEach((time) -> {
