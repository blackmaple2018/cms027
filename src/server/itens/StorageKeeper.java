...
public class StorageKeeper {
    ...
    private StorageKeeper(int id, byte slots, int meso, int accountId, byte world) {
        this.id = id;
        this.slots = slots;
    ...
    public static int create(int id, int world) throws SQLException {
        ResultSet rs;

        Connection con = Start.getInstance().getConnection();
        try (PreparedStatement ps = con.prepareStatement("INSERT INTO storages (accountid, slots, meso, world) VALUES (?, ?, ?, ?)", Statement.RETURN_GENERATED_KEYS)) {
            ps.setInt(1, id);
            ps.setInt(2, 4);
            ps.setInt(3, 0);
            ps.setByte(4, (byte) world);
            ps.executeUpdate();
            int storageid;
    ...
    public static StorageKeeper loadStorage(int id, int world) {
        ...
        try {
            ...
            ps.setInt(1, id);
            ps.setByte(2, (byte) world);
            ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                storeId = rs.getInt("storageid");
                ret = new StorageKeeper(storeId, rs.getByte("slots"), rs.getInt("meso"), id, (byte) world);
                rs.close();
                ps.close();
                ...
                storeId = create(id, world);
                ret = new StorageKeeper(storeId, (byte) 4, 0, id, (byte) world);
                rs.close();
                ps.close();
            }
    ...
    public void saveToDB() {
        ...
        try {
            con = Start.getInstance().getConnection();
            try (PreparedStatement ps = con.prepareStatement("UPDATE storages SET slots = ?, meso = ? WHERE storageid = ? AND world = ?")) {
                ps.setInt(1, slots);
                ps.setInt(2, meso);
                ps.setInt(3, id);
                ps.setByte(4, world);
    ...
    private List<Item> filterItems(InventoryType type) {
        ...
        items.stream().filter((item) -> (ItemConstants.getInventoryType(item.getItemId()) == type)).forEach((item) -> {
            ret.add(item);
        });
        return ret;
    }
    ...
    public void sendStorage(Client c, int npcId) {
        ...
        items.stream().map((item) -> ItemConstants.getInventoryType(item.getItemId())).forEach((type) -> {
            typeItems.put(type, new ArrayList<>(filterItems(type)));
        });
        c.write(PacketCreator.GetStorage(npcId, slots, items, meso));
    }

    public void sendStored(Client c, InventoryType type) {
        c.write(PacketCreator.StoreStorage(slots, type, items));
    }

    public void sendTakenOut(Client c, InventoryType type) {
        c.write(PacketCreator.TakeOutStorage(slots, type, items));
    }
    ...
    public void sendMeso(Client c) {
        c.write(PacketCreator.MesoStorage(slots, meso));
    }
    ...
    public void update(Client c) {
        //c.write(PacketCreator.ArrangeStorage(slots, items, true));
    }
