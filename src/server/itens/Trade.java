...
public class Trade {
    ...
    public final void CompleteTrade() {
        ...
        exchangeMeso = 0;
        chr.get().getClient().write(InteractionPackets.GetTradeCompletion(tradingslot));
    }
    ...
    public final void cancel(final Player chr, final int unsuccessful) {
        ...
        meso = 0;
        chr.getClient().write(InteractionPackets.GetTradeCancel(tradingslot));
    }
    ...
    public final void setMeso(final int meso) {
        ...
        if (chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            chr.get().getClient().write(InteractionPackets.GetTradeMesoSet((byte) 0, this.meso));
            if (partner != null) {
                partner.getChr().getClient().write(InteractionPackets.GetTradeMesoSet((byte) 1, this.meso));
            }
        }
    }
    ...
    public final void addItem(final Item item) {
        ...
        items.add(item);
        chr.get().getClient().write(InteractionPackets.GetTradeItemAdd((byte) 0, item));
        if (partner != null) {
            partner.getChr().getClient().write(InteractionPackets.GetTradeItemAdd((byte) 1, item));
        }
    }

    public final void chat(final String message) {
        chr.get().getClient().write(InteractionPackets.GetPlayerShopChat(chr.get(), message, true));
        if (partner != null) {
            partner.getChr().getClient().write(InteractionPackets.GetPlayerShopChat(chr.get(), message, false));
        }
    }
    ...
    public final boolean setItems(final Client c, final Item item, byte targetSlot, final int quantity) {
        ...
        if (ii.isDropRestricted(item.getItemId())) {
            c.write(PacketCreator.ServerNotice(1, "该物品无法进行操作。"));
    ...
    public final static void completeTrade(final Player p) {
        ...
        local.locked = true;
        partner.getChr().getClient().write(InteractionPackets.GetTradeConfirmation());
    ...
    public static final void startTrade(final Player c) {
        if (c.getTrade() == null) {
            ...
            c.getClient().write(InteractionPackets.GetTradeStart(c.getClient(), c.getTrade(), (byte) 0));
        } else {
            c.getClient().write(PacketCreator.ServerNotice(5, "你当前处于交易状态。"));
        }
    }
    //申请交易
    public static final void inviteTrade(final Player c1, final Player c2) {
        ...
        c1.getClient().write(PacketCreator.ServerNotice(5, "向 "+ c2.getName()+" 申请交易。"));
        c2.getClient().write(PacketCreator.ServerNotice(5, ""+ c1.getName()+" 申向你请交易。"));
        ...
        if (c2 != null && c2.getTrade() == null) {
            ...
            c2.getClient().write(InteractionPackets.GetTradeInvite(c1));
        } else {
            c1.getClient().write(PacketCreator.ServerNotice(5, "另一个玩家已经在和其他人交易了。"));
            cancelTrade(c1.getTrade(), c1);
        }
    }
    ...
    public static final void visitTrade(final Player c1, final Player c2) {
        ...
        if (c1.getTrade() != null && c1.getTrade().getPartner() == c2.getTrade() && c2.getTrade() != null && c2.getTrade().getPartner() == c1.getTrade()) {
            ...
            c2.getClient().write(InteractionPackets.GetTradePartnerAdd(c1));
            c1.getClient().write(InteractionPackets.GetTradeStart(c1.getClient(), c1.getTrade(), (byte) 1));
        } else {
            c1.getClient().write(PacketCreator.ServerNotice(5, "另一个玩家已经关闭了交易。"));
        }
    }
        ...
        if (trade != null) {
            if (trade.getPartner() != null) {
                ...
                if (other != null && other.getTrade() != null) {
                    ...
                    other.setTrade(null);
                    other.dropMessage(5, c.getName() + " 已拒绝您的交易请求。");
                }
            }
            trade.cancel(c);
            c.setTrade(null);
