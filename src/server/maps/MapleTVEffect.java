...
public class MapleTVEffect {
    ...
    private void broadcastTV(boolean activity) {
        active = activity;
        if (active) {
            BroadcastService.broadcastMessage(user.getWorldId(), PacketCreator.EnableTV());
            BroadcastService.broadcastMessage(user.getWorldId(), PacketCreator.SendTV(user, message, type <= 2 ? type : type - 3, partner));
            int delay = 15000;
            if (type == 4) {
                delay = 30000;
            } else if (type == 5) {
            ...
            BroadcastService.broadcastMessage(user.getWorldId(), PacketCreator.RemoveTV());
        }
    }
}
