...
public class FieldDoorObject extends AbstractMapleFieldObject {
    ...
    public FieldDoorObject(int owner, Field destination, Field origin, boolean town, Point targetPosition, Point toPosition, int townPortalId) {
        super();
    ...
    public void warp(final Player chr, boolean toTown) {
        if (chr.getId() == ownerId || (chr.getParty() != null && chr.getParty().getMemberById(ownerId) != null)) {
            if (chr.getParty() == null && (to.isLastDoorOwner(chr.getId()) || toTown)) {
                chr.changeMap(to, linkedPortalId);
            } else {
    ...
    public void sendSpawnData(Client client) {
        if (from.getId() == client.getPlayer().getMapId()) {
            if (client.getPlayer().getParty() != null && (ownerId == client.getPlayer().getId() || client.getPlayer().getParty().getMemberById(ownerId) != null)) {
                client.announce(PartyPackets.PartyPortal(linkedPortalId, this.getFrom().getId(), this.getTo().getId(), this.toPosition()));
            }
            ...
            client.announce(PacketCreator.SpawnDoor(this.getOwnerId(), this.getPosition(), inTown()));
        }
    }
    ...
    public void sendDestroyData(Client client) {
        if (from.getId() == client.getPlayer().getMapId()) {
            if (client.getPlayer().getParty() != null && (ownerId == client.getPlayer().getId() || client.getPlayer().getParty().getMemberById(ownerId) != null)) {
                client.announce(PartyPackets.PartyPortal(linkedPortalId, MapConstants.NULL_MAP, MapConstants.NULL_MAP, new Point(-1, -1)));
            }
