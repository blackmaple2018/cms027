...
public class PlayerShop extends AbstractMapleFieldObject {
    ...
    public void removeVisitors() {
        ...
        try {
            try {
                for (int i = 0; i < 3; i++) {
                    if (visitors[i] != null) {
                        visitors[i].getClient().announce(PersonalShopPackets.ShopErrorMessage(2, i + 1));
    ...
    public void sendShop(Client c) {
        visitorLock.lock();
        try {
            c.write(PersonalShopPackets.GetPlayerShop(this, isOwner(c.getPlayer())));
        } finally {
            visitorLock.unlock();
        }
    }
    ...
    public void sendDestroyData(Client client) {
        client.write(PersonalShopPackets.RemoveCharBox(this.getOwner()));
    }

    @Override
    public void sendSpawnData(Client client) {
        client.write(PersonalShopPackets.AddCharBox(this.getOwner(), 4));
    }
                        ...
                        if (canBuy(c, newItem)) {
                            ...
                            if (pItem.getBundles() < 1) {
                                ...
                                if (++boughtnumber == items.size()) {
                                    ...
                                    this.setOpen(false);
                                    this.closeShop();
                                    owner.announce(PersonalShopPackets.ShopCannotBuyMessage((byte) 1));
                                    //owner.dropMessage(1, "Your items are sold out, and therefore your shop is closed.");
                                }
                            }
                        } else { 
                            c.announce(PersonalShopPackets.ShopCannotBuyMessage((byte) 5));
                            //c.getPlayer().dropMessage(1, "Your inventory is full. Please clean a slot before buying this item.");
                        }
                    } else {
                        c.announce(PersonalShopPackets.ShopCannotBuyMessage((byte) 2));
