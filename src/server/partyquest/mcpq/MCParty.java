...
public class MCParty {
    ...
    public MCParty(MapleParty party) {
        ...
        party.getMembers().stream().filter((chr) -> chr.isOnline()).map((chr) -> chr.getPlayer().getChannelServer().getPlayerStorage().getCharacterById(chr.getId())).forEachOrdered((c) -> {
            characters.add(c);
        });
    }
    ...
    public void startBattle() {
        characters.forEach((chr) -> {
            chr.getClient().write(CarnivalPackets.StartMonsterCarnival(chr));
        });
    }
